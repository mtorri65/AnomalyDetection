...
for(location in locations)
{
  ...
  distanceMatrices = list.files(path = paste(".\\", location, sep = ""), pattern = "^DistanceMatrix")
  
  for(distanceMatrix in distanceMatrices)
  {
    day = substr(distanceMatrix, nchar("DistanceMatrix") + 1, nchar(distanceMatrix) - 4)
    fileName = paste(location, "\\DistanceMatrix", day, ".csv", sep = "")
