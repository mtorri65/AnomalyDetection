...
for(location in locations)
{
  ...
  distanceMatrices = list.files(path = paste(".\\", location, sep = ""), pattern = "^DistanceTimeMatrix")
  
  for(distanceMatrix in distanceMatrices)
  {
    day = substr(distanceMatrix, nchar("DistanceTimeMatrix") + 1, nchar(distanceMatrix) - 4)
    fileName = paste(location, "\\DistanceTimeMatrix", day, ".csv", sep = "")
    tb <- read.csv(fileName, header=TRUE);
    
    s = substr(location, 3, nchar(location) - 4)
    ...
    jpeg(paste(location, "\\time_", substr(location, 3, nchar(location) - 4), "_", day, ".jpg", sep = ""), width = 2000, height = 1200)
